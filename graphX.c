...
void error_handler(int error_no, int mode){
	endwin();
	switch(error_no) {
		case ERR_CREATE_SOCKET: perror("Error creating socket"); break;
		case ERR_CLOSING_SOCKET_FAILED: perror("Closing socket failed!"); break;
		...
		case ERR_CONNECTION_LOST: perror("Error: connection to peer lost!");
